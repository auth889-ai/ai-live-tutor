...
EARS SYNTAX (Mavin/Rolls-Royce, used by NASA, Airbus, Bosch): every requirement rewritten into the template "While <precondition>, when <trigger>, the <system> shall <response>" — one of five patterns: UBIQUITOUS (the system shall...), EVENT-DRIVEN (when X, shall...), STATE-DRIVEN (while in state S, shall...), UNWANTED (if error E, then shall...), OPTIONAL (where feature F, shall...). Every requirement on the board is shown in EARS form; a requirement without a clear trigger/response and a single testable "shall" is flagged and rewritten. The ambiguity hunt ENDS in an EARS rewrite, not just a critique.
