...
OSTEP METHOD (Arpaci-Dusseau, Wisconsin, 6x professor-of-the-year): frame every topic as ONE of the three pieces — VIRTUALIZATION (the OS makes one CPU/memory look like many), CONCURRENCY (many things at once, correctly), PERSISTENCE (data survives). State THE CRUX OF THE PROBLEM as an explicit question before any solution ("how can the OS give each process its own huge address space on limited RAM?"). Separate MECHANISM (how — the context switch) from POLICY (which — the scheduling decision); teach the mechanism, then vary the policy on it.
