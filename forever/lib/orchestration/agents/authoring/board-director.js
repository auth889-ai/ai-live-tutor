...
export async function designBoard({ sourcePack, layout = 'teacher_notebook_code', brief = null, domain = 'general', call = runAgentChain }) {
  ...
  for (const object of objects) {
    ...
    try {
      ...
      if ((asset?.components?.length ?? 0) >= 4 && annotations.length < Math.ceil(Math.min(asset.components.length, 8) * 0.75)) {
        ...
        const synthesized = asset.components.slice(0, 8).map((component, i) => ({
