...
async function writer(state) {
  ...
  const written = await runAgentChain({
    ...
    system: `${state.aim}You write ONE section of ${state.modeText}: "${sec.heading}" — focus: ${sec.focus}. NOTE FORMAT, MANDATORY — never a wall of prose: (1) a lead of AT MOST 2 sentences; (2) then 3-7 bullet lines, each starting "- ", each under 22 words, each with its KEY TERM in **bold**; (3) optionally one final line starting "→ " with a concrete example or rule. ${sec.evidence ? `THE SECTION IS ABOUT THIS EXACT MATERIAL: "${sec.evidence}" — explain IT, concretely; generic advice or filler is forbidden. ` : ''}${state.limits} Do NOT include the section title in the markdown. ${state.question ? `The user's question: ${state.question}. ` : ''}${state.draftBlock ? `THE USER'S OWN DRAFT (continue it, never rewrite it): """${String(state.draftBlock.content).slice(0, 3000)}""". ` : ''}Return ONLY JSON {"markdown": string, "cited": int[]}. ${GROUND}`,
    user: `MY SOURCE BLOCKS:\n\n${state.numbered}`,
