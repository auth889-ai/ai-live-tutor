// server/services/connectLearning/conceptCandidate.service.js
...
function looksLikeSentenceFragment(line = "") {
  ...
  if (/[.?!]$/.test(value) && words.length > 7) return true;
...
function isStrongConceptTitle(line = "") {
  ...
  if (!value || value.length < 4 || value.length > 110) return false;
  ...
  const hasConceptNoun =
    /\b(concept|theory|method|process|workflow|approach|model|system|architecture|practice|principle|rule|type|types|category|change|changes|migration|script|refactoring|schema|data|tool|framework|design|pattern|algorithm|protocol|security|energy|network|database|testing|deployment|integration|version|control|phase|step|formula|law|rule|module|component|strategy|technique|procedure|relationship|implementation)\b/i.test(
      value
    );
...
function classifyNodeType(title = "", evidence = "") {
  ...
  if (
    /example|case study|user story|scenario|previously|now|sample|for instance|e\.g\.|sql|alter table|create table|drop column|insert into|update .* set|code example|implementation/i.test(
      text
    )
  ) {
    return "example";
  ...
  if (/workflow|process|step|phase|approach|pipeline|procedure|lifecycle|flow|sequence/i.test(text)) {
    return "process";
  }

  if (
    /tool|framework|library|platform|liquibase|active record|flyway|docker|kubernetes|react|django|spring|postgres|mysql|mongodb|git|github|jenkins/i.test(
      text
    )
  ) {
    return "tool";
  ...
  if (/warning|risk|pitfall|mistake|anti-pattern|avoid|problem|danger|conflict|break|out-of-sync|error-prone/i.test(text)) {
    return "warning";
  }

  if (/practice|practices|should|must|recommended|best practice|guideline|principle/i.test(text)) {
    return "practice";
  }
...
function evidenceAfterLine(lines = [], index = 0, maxLines = 8) {
  ...
  for (let i = index + 1; i < Math.min(lines.length, index + maxLines + 1); i += 1) {
    ...
    selected.push(line);
    if (selected.join(" ").length > 1000) break;
  }
...
function evidenceAroundTitle(title = "", chunk = {}) {
  ...
  if (idx >= 0) {
    return cleanSpace(text.slice(idx, idx + 1300));
  }

  return cleanSpace(text.slice(0, 1000));
}
...
function scoreCandidate(candidate = {}) {
  ...
  if (candidate.title.length >= 5 && candidate.title.length <= 90) score += 1;
...
function makeCandidate({
  ...
  const candidate = {
    ...
    nodeTypeGuess: type,
    source,
    summary: trunc(quote, 340),
    pdfEvidence: trunc(quote, 1100),
    pageNumber,
    chunkId,
    pageRefs: [
      {
...
function detectDefinitionCandidates(chunks = [], visualPages = []) {
  ...
  const patterns = [
    /(?:^|\n)\s*([A-Z][A-Za-z0-9()/:+\- ]{3,90})\s+(?:is|are|means|refers to|consists of|involves|includes|requires)\s+([^.\n]{20,260})/g,
    /(?:^|\n)\s*([A-Z][A-Za-z0-9()/:+\- ]{3,90})\s*[:\-]\s+([^.\n]{20,280})/g,
  ];
  ...
  for (const chunk of chunks) {
    ...
    for (const pattern of patterns) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        const evidence = cleanSpace(`${match[1]} ${match[0]}`.slice(0, 1100));
...
function detectProcessCandidates(chunks = [], visualPages = []) {
  ...
  for (const chunk of chunks) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (
        /\b(step|phase|workflow|process|pipeline|approach|procedure|migration script|sequence|lifecycle)\b/i.test(
          line
        ) &&
      ...
      const phaseMatch = line.match(
        /\b(three[-\s]?phase|two[-\s]?phase|multi[-\s]?phase)\s+([A-Za-z ]{3,70})/i
      );
...
function detectExampleCandidates(chunks = [], visualPages = []) {
  ...
  for (const chunk of chunks) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      const isExampleArea =
        /\b(example|user story|scenario|case study|previously|now|alter table|create table|drop column|create index|update .* set|insert into|sample)\b/i.test(
          line
        );
...
function detectToolCandidates(chunks = [], visualPages = []) {
  ...
  const toolPattern =
    /\b(Liquibase|Active Record Migrations|Flyway|Prisma|Alembic|Django Migrations|Rails Migrations|Docker|Kubernetes|React|Node\.js|MongoDB|PostgreSQL|MySQL|Git|GitHub|Jenkins|CI\/CD|LangChain|LangGraph|Pinecone|Qdrant|Ollama|Tesseract|EasyOCR|PaddleOCR)\b/g;
  ...
  for (const chunk of chunks) {
    ...
    while ((match = toolPattern.exec(text)) !== null) {
      const title = cleanSpace(match[1]);
      const evidence = cleanSpace(text.slice(Math.max(0, match.index - 280), match.index + 750));
...
function detectWarningCandidates(chunks = [], visualPages = []) {
  ...
  for (const chunk of chunks) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (
        !/\b(problem|risk|warning|mistake|pitfall|conflict|break|out-of-sync|error-prone|avoid|danger)\b/i.test(
          line
        )
      ) {
        continue;
...
function detectVisualCandidates(chunks = [], visualPages = []) {
  ...
  for (const visual of visualPages) {
    ...
    const title =
      visual.title ||
      (visual.visualType
        ? `${visual.visualType} on page ${visual.pageNumber}`
        : `Visual example page ${visual.pageNumber}`);

    const visualText = cleanSpace(
      [visual.title, visual.summary, visual.ocrText, matchingChunk?.text ? trunc(matchingChunk.text, 650) : ""]
        .filter(Boolean)
        .join(" ")
    );

    const nodeTypeGuess = /table|code|screenshot|example|sql/i.test(`${visual.visualType} ${visualText}`)
      ? "example"
