...
export default function ProgressPage() {
  ...
  return (
    <Shell done={done} active={items.length - done}>
      {items.length === 0 ? (
...
function Shell({ children, done = 0, active = 0 }) {
  return (
    <div style={{ maxWidth: 780, margin: '0 auto', padding: '32px 16px' }}>
      ...
      <p style={{ color: '#8a6d3b', fontSize: 13.5, marginBottom: 18 }}>
        {done + active > 0 ? `${active} in progress · ${done} completed — bars are earned by finishing scenes, not by opening them.` : 'Bars are earned by finishing scenes, not by opening them.'}
      </p>
      {children}
    </div>
  );
